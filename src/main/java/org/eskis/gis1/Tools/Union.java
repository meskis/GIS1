...
import org.eskis.gis1.Tools.WrappingIterator;

    /**
     * Delegate that will compute the intersections on the go
     */
    class UnionFeatureCollection extends DecoratingSimpleFeatureCollection {

        SimpleFeatureCollection features;
        SimpleFeatureType type;

        public UnionFeatureCollection(SimpleFeatureCollection delegate,
                SimpleFeatureCollection features) {
            super(delegate);
     ...
     */
    class UnionFeatureIterator implements SimpleFeatureIterator {
        ...
        public UnionFeatureIterator(SimpleFeatureIterator delegate,
                        ...
                        try {
                            while (iterator.hasNext()) {
                                ...
                                if (currentGeom.getEnvelope().intersects(
                                    ...
                                    try {
	                                    attribute = currentGeom.union((Geometry) second
	                                            .getDefaultGeometry());
                                	} catch (Throwable t){
                                		//t.printStackTrace();
                                		continue;
                                	...
                                	//try {
	                                    attribute = first.getDefaultGeometry();//currentGeom.union((Geometry) second
	                                            //.getDefaultGeometry());
                                	//} catch (Throwable t){
                                		//t.printStackTrace();
                                		//continue;
